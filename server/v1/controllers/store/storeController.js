...
import storeOtps from "./storeOtp";
...
class storeController {
  async login(req, res, next) {
    try {
      ...
      let user = await model.store.findOne(qry, {
        hash: 1,
      });
      if (!user) {
  ...
  async setPassword(req, res, next) {
    try {
      ...
      await model.store.findByIdAndUpdate(req.user._id, {
        $set: {
  ...
  async uploadDocument(req, files, res, next) {
    try {
      ...
      let store = await model.store.findByIdAndUpdate(
        req.query.store,
        {
          $set: {
            ownerId: ownerId,
  ...
  async getStoresUploadedDocuments(req, res, next) {
    try {
      let store = await model.store.findByIdAndUpdate(req.query.id);
      return res.success("OK", store);
    } catch (error) {
      next(error);
    }
  ...
  async verifyDocuments(req, res, next) {
    try {
      ...
      if (type === "ownerIdStatus") {
        store = await model.store.findByIdAndUpdate(
          req.body.id,
          {
            $set: {
  ...
  async signup(req, res, next) {
    try {
      ...
      let find_store;
      if (req.body.email) {
        find_store = await model.store.findOne({
  ...
  async resendOtp(req, res, next) {
    ...
    if ((req.body.key && (await storeOtp.isEmail(req.body.key))) || req.body.email) {
      ...
      let user;
      if (req.body.id) {
        user = await model.store.findById(mongoose.Types.ObjectId(req.body.id));
        email = user.email;
      }
  ...
  async resetPassword(req, res, next) {
    ...
    if ((req.body.key && storeOtp.isEmail(req.body.key)) || req.body.email) {
      let email = req.body.key || req.body.email;
      let user = await model.store.findOne({
  ...
  async verifyOtp(req, res, next) {
    ...
    if (req.body.key && (await storeOtp.isEmail(req.body.key))) {
      ...
      let resultUser = await model.store.findOneAndUpdate(qry, {
  ...
  async notifications(req, res, next) {
    try {
      ...
      pipeline.push({
        $match: {
          storeId: mongoose.Types.ObjectId(req.query.store),
        },
      });
      pipeline.push({
        $sort: {
  ...
  async getEarningList(req, res, next) {
    try {
      ...
      let data = await model.store.aggregate(pipeline);
      let count = data.length;
      ...
      data = await model.store.aggregate(pipeline);
  ...
  async getNewStore(req, res, next) {
    try {
      ...
      let orders = await model.store.aggregate(pipeline);
      // let orders = await model.store.find(filter);
  ...
  async areaWiseSale(req, res, next) {
    try {
      ...
      let data = await model.storeOrder.aggregate(pipeline);
      return res.success("OK", data);
    } catch (error) {
      next(error);
    }
  ...
  async selfDelivery(req, res, next) {
    try {
      ...
      let data = await model.storeOrder.findByIdAndUpdate(
        {
          _id: mongoose.Types.ObjectId(req.body.id),
        },
        {
...
console.log("In Status Change",data);
  ...
  async getEarningGraph(req, res, next) {
    try {
      ...
      let totalSale = await model.store.aggregate(pipeline);
      ...
      if (req.query.type === "weekly") {
        ...
        let weakData = await model.store.aggregate(pipelineEarning);
        let finalWeekData = [];
        ...
        let monthData = await model.store.aggregate(pipelineEarning);
      ...
      let user_earning = await model.storeOrder.aggregate(pipeline_user);
  ...
  getStoreCouponList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let storeIds = await model.storeItem.find({}).distinct("storeId");
  ...
  getCategoryCouponList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let categoryIds = await model.storeItem.find({}).distinct("storeItemTypeId");
  ...
  getSubCategoryCouponList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let subCategoryIds = await model.storeItem.find({}).distinct("storeItemSubTypeId");
  ...
  getBrandCouponList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let brandIds = await model.storeItem.find({}).distinct("brandId");
  ...
  getProductCouponList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let itemIds = await model.storeItem.find({}).distinct("productKey");
  ...
  getProductDealsList(lang) {
    return new Promise(async (done, reject) => {
      try {
        let itemIds = await model.storeItem.find({}).distinct("_id");
  ...
  async homeData(data, userId, lang, user) {
    try {
      return new Promise(async (done, reject) => {
        ...
        storeItemBrandIds = await model.storeItem.distinct("brandId", qry);
        let storeBrands;
        qry = {
          $and: [
            {
        ...
        storeBrands = await model.brand.find(qry).lean().exec();
        let deals = [];
        qry = {
          code: "DEAL",
          status: {
        ...
        for (let i = 0; i < deals.length; i++) {
          outlet = await model.storeOutlet.findOne({
        ...
        // let outlets = model.storeOutlet.

        /*********************BEST OFFERS****** */
        let bestOffers = this.getBestOffers(data, 0, 5);

        /****************************RECOMMENDED*********** */
        let recommended = await this.getRecommendedStores(data, 0, 5);
        ...
        storeTypes = model.storeCategory
          .find(qry)
          .sort({
            serialNo: 1,
          })
        ...
        // let storeTypes = await model.storeCategory.find().lean();
        ...
        for (let i = 0; i < storeTypes.length; i++) {
          ...
          let store = await model.store.aggregate([
            {
              $match: {
                storeTypeId: storeTypes[i]._id,
                $and: [
...
//                isVisible: true,
  ...
  async getSavedStores(data, skip, limit) {
    ...
    /*************************** FILTERING LOGIC END***************** */
    return await model.store.aggregate(pipeline).exec();
  }
  async getNearByOutlets(data) {
    ...
    if (data.isGeofenceActive) qry.geofenceId = data.geofenceId;
    let outlets = await model.store.aggregate([
      {
        $match: qry,
      },
      {
  ...
  async getStoresByProduct(data) {
    ...
    stores = await model.store
      ...
      .exec();
    let storeType = await model.storeCategory.findById(stores[0].storeTypeId);
    return {
      data: {
  ...
  async searchNew(data) {
    ...
    if (data.latitude != null && data.longitude != null) {
      let storeIdData = await model.store.aggregate([
        {
          $lookup: {
            from: "storeoutlets",
            let: {
    ...
    let response = [];
    let data1 = await Promise.all([products, stores, brands, storeItemType, storeItemSubType]);
    products = data1[0];
    brands = data1[2];
    stores = data1[1] ? data1[1] : null;
    storeItemType = data1[3];
    storeItemSubType = data1[4];
  ...
  async getBestOffers(data, skip, limit) {
    try {
      ...
      let pipeline = [
        ...
        {
          $lookup: await this.itemLookup(data),
        },
        {
          $match: {
            items: {
      ...
      pipeline.push(
        ...
        {
          $lookup: await this.dealLookup(data.userId),
        },
        {
          $match: {
            promocodes: {
      ...
      let returnDatata = await model.store.aggregate(pipeline).exec();
      return returnDatata;
    } catch (error) {
      console.log(error, "ssfsdfssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss");
    }
  }
...
console.log("result:",data.userId);
    let qry = {
      ...
      isRecommended: 1,
//Not Found in the DB 29-Dec-2021 03:13 AM
//      isVisible: true,
      isHyperLocal: false,
    };
    if (data.moduleKey) {
    ...
    /*************************** FILTERING LOGIC END***************** */
    let responseData = await model.store.aggregate(pipeline).exec();
...
//      isVisible: true,
    ...
    const beforeSevenDays = new Date(moment().subtract(7, "d").startOf("date"));
    const today = new Date(moment().endOf("date"));
    if (data.newSort == "newFirst")
      pipeline.push(
        {
          $lookup: {
  ...
  async storeData(storeId, data) {
    ...
    let store = await model.store
      ...
      .exec();
    let storeNew = await model.store.findById(storeId).lean();
    let storeType = await model.storeCategory.findById(storeNew.storeTypeId).lean();
    let category = await model.storeItemType
      .find({
    ...
    for (let i = 0; i < category.length; i++) {
      let subCategory = await model.storeItemType
        .find({
      ...
      if (subCategory && subCategory.length) {
        for (let j = 0; j < subCategory.length; j++) {
          ...
          let products = await model.storeItem.aggregate(pipeline);
  ...
  async getCategories(data, skip, limit) {
    ...
    storeTypes = await model.storeCategory.find(qry).lean().exec();
    let emptyStoreTypesIndices = [];
    for (let i = 0; i < storeTypes.length; i++) {
      if (storeTypes[i].isHyperLocal) {
        let store = await model.store.aggregate([
  ...
  async allCategories(data, userId, lang) {
    return new Promise(async (done, reject) => {
      ...
      let count = await model.storeCategory.countDocuments({
        $and: [
          {
            status: {
              $ne: 2,
  ...
  async viewAllProducts(data, userId, lang) {
    ...
    for (const product of products) {
      let dealExists = await model.promocode.findOne({
        code: "DEAL",
        $or: [
          {
            subCategoryId: {
              $in: [mongoose.Types.ObjectId(product.storeItemSubTypeId)],
            },
          },
          {
            categoryId: {
              $in: [mongoose.Types.ObjectId(product.storeItemTypeId)],
            },
          },
          {
            brandId: {
              $in: [mongoose.Types.ObjectId(product.brandId._id)],
            },
          },
        ],
        store: mongoose.Types.ObjectId(data.storeId),
        status: {
          $in: [1, 3],
        },
        startDate: {
      ...
      for (const variant of product.variants) {
        if (dealExists) {
          if (dealExists.discountType.toLowerCase() === "flat") {
            variant.price = Number(variant.price - dealExists.discount);
          } else {
            variant.price = variant.price - ((dealExists.discount * 1) / 100) * variant.price;
          }
        } else {
          let variantDealExists = await model.promocode.findOne({
            code: "DEAL",
            itemId: {
              $in: [mongoose.Types.ObjectId(variant._id)],
            },
            store: mongoose.Types.ObjectId(data.storeId),
            status: {
              $in: [1, 3],
          ...
          if (variantDealExists) {
            if (variantDealExists.discountType.toLowerCase() === "flat") {
              variant.price = Number(variant.price - variantDealExists.discount);
            } else {
              variant.price = variant.price - ((variantDealExists.discount * 1) / 100) * variant.price;
            }
          }
        }
      }
  ...
  async storeDetail(storeId, queryData, userId, lang) {
    return new Promise(async (done, reject) => {
      ...
      let { store, brands } = data.data;
      let storeOpen = await model.store.findById(storeId);
  ...
  async storeDetail2(storeId, data, userId, lang) {
    return new Promise(async (done, reject) => {
      ...
      let cats = (async () => {
        let cats = await model.storeItem.find(filter).distinct("storeItemTypeId").exec();
        cats = await model.storeItemType
          .find({
            _id: {
              $in: cats,
            },
      ...
      let brands = (async () => {
        let brands = await model.storeItem.find(filter).distinct("brandId");
      ...
      /* let promotionalItems = (async () => {
        let productKeys = await model.storeItem.find(filter).distinct("productKey");
        ...
        //   }
        // }
        promotionalItems = await model.storeItem.aggregate([
          {
            $match: {
  ...
  async getProductsByCategory(data, userId) {
    if (data.dynamicLayout == "one" || data.dynamicLayout == "three" || data.dynamicLayout == "two" || data.dynamicLayout == "four") {
      try {
        let qry = [
          {
	...
	let storeProperty = await model.store.findById(data.storeId,'dineIn driveThru delivery schedule takeAway').exec();
...
let sortBy = "price";
	...
	criteria.visible=true;
        ...
        if (data && data.label && data.label != "all") filter.label = data.label;
        let subCategory = await model.storeItem.distinct("storeItemSubTypeId", filter);
	console.log("Record of SubCategory",subCategory);        
if (data.dynamicLayout == "two") {
          let itemData = [];
          let pipeline2 = [
            {
              $match: filter,
          ...
          let itemIdssss = await model.storeItem.aggregate(pipeline2);
          if (itemIdssss.length > 0) {
            for (const i of itemIdssss) {
              if (i != null) {
                itemData.push(i._id);
              }
            }
          ...
          subCategory = await model.storeItem.distinct("storeItemSubTypeId", filter);
        }
        ...
        subCategory = await model.storeItemType.aggregate(subCategoryPipeline);
        let promos = await model.promocode
          .find({
            code: {
              $ne: "DEAL",
            },
        ...
        const promoProducts = await model.storeItem.aggregate([
          {
            $match: {
              storeId: mongoose.Types.ObjectId(data.storeId),
            },
      ...
      for (var i = 0; i < deals.length; i++) result = [...result, ...deals[i].productId];
      let promotionalItems = await model.storeItem.aggregate([
        {
          $match: qry1,
        },
        {
  ...
  async getProductsByNoChildCategory(data, userId) {
    ...
    let finaldata = await model.storeItem.aggregate(pipeline);
  ...
  async favProductsList(data, userId) {
    let products = await model.favProduct
      .find({
        userId,
      })
      .distinct("productKey");
    ...
    if (data.isGeofenceActive && (data.isGeofenceActive == true || data.isGeofenceActive == "true")) {
      ...
      if (geofenceData != null) {
        pipeline.push({
          $match: {
            "brandId.geofenceId": geofenceData._id,
          },
        });
      }
    }
    products = await model.storeItem.aggregate(pipeline);
    return {
      data: products,
    };
  }
      ...
      rating
        ...
        .then((result) => {
          ...
          if (data && data.itemRating && data.itemRating.length > 0) {
            let irating = new model.storeItemRating(data);
  ...
  async createOrder(data, userId, lang) {
	...
	console.log("In createOrder");
        ...
        const cartDetails = await model.storeCart.find({
          userId: userId,
        });
        let geofenceId = null;
        if (cartDetails.length > 0) {
          let itemDataGeofence = await model.storeItem.findById(cartDetails[0].itemId);
        ...
        if (appSetting.loyalityPointsType == "ORDER") {
          ...
          cartDetails.forEach(async (x) => {
            // let item = await model.storeItem.findById(x.itemId);
            loyalityPoints += x.itemQuantity * x.LP;
          });
          userDetails = await model.user
            .findOneAndUpdate(
              {
                _id: userId,
              },
        ...
        data.loyalityPoints = loyalityPoints;
        let ticketsCount = 0;
        cartDetails.forEach(async (x) => {
          let item = await model.storeItem.findById(x.itemId);
	...
	console.log("Merchant",store.country_code,store.phone_no);
        ...
        if (data.isWallet) {
          ...
          if (userDetails.wallet) {
            data.balanceLeft = data.totalAmount - wallet;
            if (data.balanceLeft < 0) {
              data.balanceLeft = 0;
              wallet = wallet - data.totalAmount;
            } else {
              wallet = 0;
            }
        ...
        if (data.couponCode != null && data.couponCode != "") {
          let promo = await model.promocode.findOne({
            code: new RegExp("^" + data.couponCode + "$", "i"),
            status: {
              $in: [1, 3],
            },
            startDate: {
          ...
          if (dailyUsePromo) {
            if (dailyUsePromo.first_time_use_in_last_24hours > Date.now() - 24 * 60 * 60 * 1000) {
        ...
        const order = await model.storeOrder(data).save();
        await model.storeOrder
          .findOne({
            _id: order._id,
          })
          .then(async (result) => {
            if (result.paymentMode === "Gpay" || data.isWallet) {
              await model
                .Transaction({
                  userId: data.userId,
            ...
            if (store.storePackageType.toLowerCase() === "percentage") {
              let adminCommission =
                Number(result.subTotalAmount * store.storePackageTypeValue * 0.01) +
                Number(result.deliveryFee) +
                Number(result.tax) +
                Number(result.packagingCharge);
              adminCommission = adminCommission ? adminCommission : 0;
            ...
            if (store.storePackageType.toLowerCase() === "flat") {
              let adminCommission = Number(result.subTotalAmount) + Number(result.deliveryFee) + Number(result.tax) + Number(result.packagingCharge);
              adminCommission = Number(adminCommission) ? Number(adminCommission) : 0;
            ...
            if (store.storePackageType.toLowerCase() === "membership") {
              let adminCommission = Number(result.subTotalAmount) + Number(result.deliveryFee) + Number(result.tax) + Number(result.packagingCharge);
              adminCommission = Number(adminCommission) ? Number(adminCommission) : 0;
              let merchantCommission = 0;
		...
		console.log("Data 3");
			...
			console.log("Data 4");
                ...
                let updateDataEmail = await model.storeOrder
                  .findOne({
                    _id: resp._id,
                  })
                  .populate("storeId")
                  .populate("storeoutlets")
                  .populate("userId")
                  .populate("driverId")
                  .populate("items.itemId");
                if (updateDataEmail.storeId != null && updateDataEmail.storeId.email != null) {
                  let payload = {
                    to: updateDataEmail.storeId.email,
                    subject: "New order created",
                    data: updateDataEmail,
                  };
			console.log("Data 5");
		...
		done({message: multilingualService.getResponseMessage("ORDERPLACED",lang),data:resp});
                // console.log(
                //   "Code",
                //   store.country_code.replaceAll("+", "") + store.phone_no
                // );
                const merchantWhatsappNo =
                  store.country_code.replaceAll("+", "") + store.phone_no;
//                let whatsappHeader = {
//                  authorization:
//                    "Bearer eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.eyJ1c2VyIjoiYWRtaW4iLCJpYXQiOjE2NDQ5MjQwOTYsImV4cCI6MTY0NTUyODg5Niwid2E6cmFuZCI6IjI3YzRkNDg4NDViNTM4NjcyNTcyNzdmMDU4YzZjNzU5In0.OoU3yOsR10EeENIUCrfOqj8-yonGmtb2vH0MXVocDxo",
 //                 "content-type": "application/json",
  //              };
                ...
                await axios({
                  ...
                  url: process.env.WHATSAPPURL + "/v1/contacts/",
                  data: {
                    contacts: [merchantWhatsappNo],
                  },
                });
  ...
  //                  whatsappHeader,
                  ...
                  url: process.env.WHATSAPPURL + "/v1/messages/",
                  data: {
                    to: merchantWhatsappNo, //store.country_code.replaceAll("+", "") + store.phone_no, //"923227526676",
                    type: "template",
                    template: {
                      namespace: process.env.WHATSAPPNAMESPACE, //"be636d5e_e263_4c97_a362_c5d95a544afd",
                      name: process.env.WHATSAPPTEMPLATENAME, //"jaak",
...
//		console.log("Inside Notification",payload2,"data",data);
//                await Service.Notification.webPushNotification(payload2);
//                done({
...
//                });
  ...
  getAllOrders(data, userId, lang) {
    return new Promise(async (done, reject) => {
      ...
      qry.status =
        data.status == "pending"
          ? {
              $in: [0, 1, 2, 3, 18, 9,6,7,8,10,15,16,17],
            }
          : {
              $nin: [0, 1, 2, 3, 18, 9,6,7,8,10,15,16,17],
            };
      let skip = Number(data.page) > 1 ? (Number(data.page) - 1) * Constant.LIMIT : 0;
      ...
      await model.storeOrder
        ...
        .lean()
        .then(async (result) => {
          await model.storeOrder.countDocuments(qry).then(async (count) => {
  ...
  getBrandById(data, userId, lang) {
    return new Promise(async (done, reject) => {
      try {
        ...
        const products = await model.storeItem.aggregate(pipeline);
        for (const product of products) {
          let dealExists = await model.promocode.findOne({
            code: "DEAL",
            $or: [
  ...
  //   let cats = await model.storeItemType
  //     .find({
  //       storeCategoryId: { $in: storeTypes },
  //     })
  //     .distinct("_id");

  //   let catResults = await model.storeItemType.aggregate([
  //     {
  //       $match: {
  //         name: { $regex: data.keyword, $options: "i" },
  ...
  //     { $limit: 2 },
  //   ]);
  //   let brandResults = await model.storeItemType.aggregate([
  //     {
  //       $match: {
  //         name: { $regex: data.keyword, $options: "i" },
  //       },
  ...
  //   let subCatResults = await model.storeItemType.aggregate([
  //     {
  //       $match: {
  //         name: { $regex: data.keyword, $options: "i" },
  ...
  //     { $limit: 2 },
  //   ]);
  //   let products = await model.storeItem.aggregate([
  //     {
  //       $match: {
  //         name: { $regex: data.keyword, $options: "i" },
  ...
  //     let storeTypesData = await model.storeCategory.find({}).lean();
  //     let storeTypesMap = {};
  ...
  //     let catData = await model.storeItemType.find({}).lean();
  ...
  //     let subCatResults = await model.storeItemType
  //       .find({
  //         name: { $regex: data.keyword, $options: "i" },
  ...
  checkOrderAddress(data, userId, lang) {
    return new Promise(async (done, reject) => {
      try {
		...
		console.log("Data",data," StoreID",storeId);

        const perKmCharge = await model.AppSetting.findOne({}).exec();
        if (storeId != null) {
          let radius = await model.store.findOne(
            {
              _id: storeId,
  ...
  async getProductById(data, lang) {
    return new Promise(async (done, reject) => {
      try {
        let deliveryRadius = Constant.RADIUSCIRCLE;
        if (data.moduleName == "ecommerce") {
          deliveryRadius = 99999999999999999;
        }
        let products = await model.storeItem.aggregate([
          {
            $match: {
        ...
        for (const x in products) {
          ...
          let store = await model.store.aggregate([
            {
              $match: {
                _id: mongoose.Types.ObjectId(products[x].storeId),
              },
            },
            {
              $lookup: {
          ...
          for (const y in store) {
            products[x]["storeAddress"] = store[y].storeOutlets[0].address || "";
            products[x]["outletId"] = store[y].storeOutlets[0]._id;
            products[x]["storeId"] = store[y]._id;
          }
        }
        let storeID;
        for (const product of products) {
          let dealExists = await model.promocode.findOne({
            ...
            store: mongoose.Types.ObjectId(product.storeId),
            status: {
              $in: [1, 3],
            },
            startDate: {
  ...
  async getProductByKeyAndStoreId(data, lang) {
    return new Promise(async (done, reject) => {
      try {
        let product = await model.storeItem.aggregate([
          {
            $match: {
              productKey: data.productKey,
              storeId: mongoose.Types.ObjectId(data.storeId),
  ...
  async getStoresByCategoryOrSubCategory(req, res, next) {
    try {
      ...
      if (data.sortType === "descending") sortType = -1;
      let sort = {};
      sortBy = `storeId.${sortBy}`;
      sort[sortBy] = sortType;
      if (data.rating && data.rating !=  null && data.rating != "") {
        data.rating = Number(data.rating);
        let ratingFilter = {
          "storeRatings.avgRating": {
            $gte: data.rating,
          },
        };
        pipeline.push({
      ...
      if (data.latitude && data.longitude) {
        const store = await model.storeOutlet
          .find({
            status: 1,
            location: {
              $near: {
      ...
      // //     },
      // //   ],
      // //   storeItemTypeId: mongoose.Types.ObjectId(products[0].storeItemTypeId),
      // //   storeItemSubTypeId : mongoose.Types.ObjectId(products[0].storeItemSubTypeId),
      // //   isVisible: true,
      // // };
      ...
      //   },
      //   {
      //     $lookup: await outletLookup1(data),
      //   },
      //   {
      //     $project: preProject,
      //   },
      ...
      //   },
      //   {
      //     $lookup: await favLookup1(data.userId),
      //   },
      //   // { $skip: skip },
      //   // { $limit: limit },
      //   {
      ...
      // let newStores = await model.storeItem.aggregate(pipeline2).exec();
...
async function findGeofenceId(longitude, latitude) {
