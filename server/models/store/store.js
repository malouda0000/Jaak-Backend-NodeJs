...
const deliveryChargesSchema = new Schema({
  isFree: { type: Boolean },
  fixed_delivery_charges: { type: Number },
  deliveryPrices: {
...
let storeSchema = new Schema(
  {
    ...
    emergency_countrycode: { type: String },
    ...
    delivery_type:[{
        type: String,
        enum: ['RECURING', 'DELIVERY', 'DRIVETHRU', 'DINEIN', 'SCHEDULE','TAKEAWAY'],
    }],
    deliveryAreaType: {
      type: String,
    ...
    isHyperLocal: { type: Boolean, default: false },
    layout: {
      type: String,
      enum: ["grid", "list"],
      default: "list",
    ...
    maxProductPrice: { type: Number, default: 0 },
    ...
    salesPersonCommission: {type : Number , default : 0},
    isVisible: Boolean,
    isOpen: { type: Boolean, default: true },
    state: { type: String, enum: ["open", "close", "auto"] },
    veg_nonveg: { type: Boolean, default: false },
...
storeSchema.pre("save", async function (next) {
  var doc = this;
  if (doc.isNew) {
      let countMerchant = await Store.countDocuments();
      let appName = await mongoose.model("AppSetting").findOne();
      if (countMerchant > 0) {
        let count = countMerchant
        doc.storeNo = `${appName.appName}_M${++count}`;
      } else {
        doc.storeNo = `${appName.appName}_M1`;
      }
    next();
  } else {
    next();
